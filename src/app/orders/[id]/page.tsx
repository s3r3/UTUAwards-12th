...
export default function OrderDetailPage() {
  ...
  if (loading) return <div className="min-h-screen pt-28 flex items-center justify-center">{t.common.loading}</div>
  if (!order) return <div className="min-h-screen pt-28 flex items-center justify-center">{t.orders.notFound}</div>
  ...
  return (
    <div className="min-h-screen pt-28 pb-16 bg-white dark:bg-gray-950">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        {/* Header */}
        <div className="mb-10">
          <button onClick={() => router.back()} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white mb-6">
            <ArrowLeft size={18} /> {t.common.back}
          </button>
          <div className="flex items-center justify-between">
            <h1 className="font-serif text-3xl font-bold text-gray-900 dark:text-white">Order #{order.id.slice(0, 8)}</h1>
            <span className={`px-4 py-2 rounded-full text-xs font-semibold uppercase tracking-widest border ${status.bg} ${status.text} ${status.border}`}>
              {t.orders.statuses[order.status as keyof typeof t.orders.statuses] || order.status}
            </span>
          </div>
          <p className="text-xs text-gray-500 font-mono tracking-wider mt-2">
            {new Date(order.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
          </p>
        </div>

        {/* Items */}
        <section className="border border-black/10 bg-white dark:bg-gray-900 divide-y divide-black/5 dark:divide-gray-800">
          <div className="px-6 py-4">
            <h2 className="font-serif text-lg font-semibold text-gray-900 dark:text-white">{t.orders.items} ({order.items.length})</h2>
          </div>
            ...
            {order.items.map(item => {
              ...
              return (
                <div key={item.id} className="flex items-center gap-4">
                  <div className="w-16 h-16 relative bg-stone-100 dark:bg-gray-800 rounded">
                    {item.product?.image ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-gray-900 dark:text-white truncate">{item.product?.name || `Produk ${item.id.slice(0, 6)}`}</p>
        ...
        {/* Address */}
        <section className="border border-black/10 bg-white dark:bg-gray-900 mt-6">
          <div className="px-6 py-4 border-b border-black/5">
            <h2 className="font-serif text-lg font-semibold text-gray-900 dark:text-white">{t.orders.address}</h2>
          </div>
          <div className="px-6 py-4">
        ...
        <section className="border border-black/10 bg-white dark:bg-gray-900 mt-6">
          <div className="px-6 py-4 border-b border-black/5">
            <h2 className="font-serif text-lg font-semibold text-gray-900 dark:text-white">Metode Pembayaran</h2>
          </div>
          <div className="px-6 py-4">
            <p className="text-sm text-gray-500 capitalize">{order.status === 'PAID' ? 'Pembayaran berhasil dikonfirmasi' : 'Sedang menunggu pembayaran'}</p>
          </div>
        </section>
        ...
        <section className="border border-black/10 bg-white dark:bg-gray-900 mt-6">
          <div className="px-6 py-4 border-b border-black/5">
            <h2 className="font-serif text-lg font-semibold text-gray-900 dark:text-white">Ringkasan</h2>
          </div>
          <div className="px-6 py-4 space-y-3">
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Subtotal ({totalItemQty} items)</span>
