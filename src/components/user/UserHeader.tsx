...
export default function UserHeader({ onMenu }: { onMenu: () => void }) {
  const pathname = usePathname()
  const { theme, setTheme } = useUIStore()
  const isDark = theme === 'dark'
  const title = titles[pathname] || 'Member Area'
  ...
  return (
    <header className="sticky top-0 z-20 flex min-h-16 items-center gap-3 border-b border-gray-200 bg-white/95 px-4 backdrop-blur dark:border-gray-800 dark:bg-gray-950/95 sm:px-6">
      ...
      <Link href="/dashboard/notifications" className="relative rounded-lg p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="Notifikasi">
        <Bell size={19} />
      </Link>
      <Link href="/products" className="hidden items-center gap-2 rounded-lg px-2 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-100 sm:flex dark:text-gray-300 dark:hover:bg-gray-800">
